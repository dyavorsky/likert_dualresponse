...
$$
where $\varepsilon_i = \eta_{i0} - u_i^*$. As is well known,[^3] $\varepsilon_i$ follows a Gumbel distribution with location parameter $\overline{\mu}_i$ and scale parameter $1$, where
$$
...
$$
p_i = F_{\text{Gumble} \left( \overline{\mu}_i, 1 \right)} \left( \varepsilon_i \right).
$$

[^3]: See, e.g., [@Mcfadden_1981] or [@Cardell_1997].
...
\begin{aligned}
    ...
    &= F_{\text{Gumbel} \left( \overline{\mu}, 1 \right) } \left( F^{-1}_{\text{Gumbel} \left( 0, 1 \right) } \left( \alpha_{w\left(i\right)} \right) \right) - 
       F_{\text{Gumbel} \left( \overline{\mu}, 1 \right) } \left( F^{-1}_{\text{Gumbel} \left( 0, 1 \right) } \left( \alpha_{w\left(i\right)-1} \right) \right) \\
